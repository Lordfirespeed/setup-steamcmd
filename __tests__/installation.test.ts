...
let extractTarMock: jest.SpyInstance<Promise<string>, [file: string, dest?: string | undefined, flags?: string | string[] | undefined]>
...
describe('action', () => {
  ...
  function mockWindowsImplementations() {
    ...
    cacheDirMock.mockImplementation(
      async(sourceDir, tool, version, arch)=> path.win32.join("[tool-cache]", tool, version, arch || "")
    )
  }
  ...
  function mockPosixImplementations() {
    ...
    cacheDirMock.mockImplementation(
        async(sourceDir, tool, version, arch)=> path.posix.join("/", "[tool-cache]", tool, version, arch || "")
    )
  }
  ...
  describe("Windows", () => {
    ...
    it('installs and tests the steamCMD tool on Windows', async () => {
      ...
      expect(cacheDirMock).toHaveBeenCalledWith(await extractZipMock.mock.results[0].value, expect.stringMatching(/^steamcmd$/i), expect.any(String), expect.any(String))
  ...
  describe("MacOS", () => {
    ...
    it('installs and tests the steamCMD tool on MacOS', async () => {
      ...
      expect(cacheDirMock).toHaveBeenCalledWith(await extractTarMock.mock.results[0].value, expect.stringMatching(/^steamcmd$/i), expect.any(String), expect.any(String))
  ...
  describe.each<NodeJS.Platform>(
      ["aix", "freebsd", "cygwin", "openbsd", "netbsd", "sunos", "android", "haiku"]
    ...
    it(`throws when platform is '${platformIdentifier}'`, async () => {
      await expect(async () => await main.attemptInstall())
          .rejects
